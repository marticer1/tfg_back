...
public class ProblemCollectionGetControllerIT {
    ...
    @Test
    void getAllProblemCollections_returnsOkResponse() throws Exception {
        mockMvc.perform(get("/problem-collections")
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray())
                .andExpect(jsonPath("$[0].id").value(collectionId.toString()))
